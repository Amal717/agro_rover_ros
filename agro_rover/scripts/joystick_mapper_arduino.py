...
def map_to_255(value, inverted):
    # Map the value from -1 to 1 to 0 to 255, with optional inversion
    if inverted:
        mapped_value = (value + 1) * 127.5
    else:
        mapped_value = (1 - value) * 127.5
    return int(max(0, min(255, mapped_value)))
...
def joystick_callback(msg):
    ...
    # Publish axis values
    axis_values = [map_to_255(msg.axes[index], inverted_axes[axis_mapping[index]]) for index in axis_mapping]
